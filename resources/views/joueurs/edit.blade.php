    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form method="POST" action="{{ route('joueurs.update', $joueur) }}" enctype="multipart/form-data" class="p-6">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="date" 
                                   id="birthday" 
                                   name="birthday" 
                                   value="{{ old('birthday', $joueur->birthday) }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
                                   required>
                            @error('birthday')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <select id="sport_id" 
                                ...
                                @foreach($sports as $sport)
                                    <option value="{{ $sport->id }}" {{ old('sport_id', $joueur->sport_id) == $sport->id ? 'selected' : '' }}>
                                        {{ $sport->nom }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="equipe_id" 
                                ...
                                @foreach($equipes as $equipe)
                                    <option value="{{ $equipe->id }}" {{ old('equipe_id', $joueur->equipe_id) == $equipe->id ? 'selected' : '' }}>
                                        {{ $equipe->nom }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-span-full">
                            ...
                            <div class="mt-2">
                                @if($joueur->image)
                                    <div class="mb-3">
                                        <img src="{{ Storage::url($joueur->image) }}" 
                                             alt="{{ $joueur->name }}" 
